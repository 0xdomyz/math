...
| **Reverse Stress** | Capital target → required scenarios | Backward looking | High | UK PRA |
...
**Edge Case:**  
Tail scenario: Systemic shock (pandemic, war, financial collapse); correlated defaults across all segments simultaneously

## 4. Layer Breakdown
```
Stress Testing Framework:
├─ Scenario Construction:
│   ├─ Macro variables: Unemployment ↑, GDP ↓, rates ↑, spreads ↑
│   ├─ Market prices: Equity ↓, FX volatility ↑, asset prices ↓
│   ├─ Credit: Default rates ↑, recovery rates ↓, rating migrations
│   ├─ Contagion: Sector spillover, counterparty defaults
│   └─ Liquidity: Haircuts ↑, bid-ask spreads ↑, funding stress
├─ Scenario Development Methods:
│   ├─ Historical: Reproduce 2008, 1998 LTCM, Asian crisis
│   ├─ Hypothetical: Design "what-if" scenarios
│   ├─ Econometric: Build macro-to-credit model
│   ├─ Expert judgment: Consensus on unlikely outcomes
│   └─ Reverse: Start with capital target, back out scenario
├─ Loss Calculation:
│   ├─ Mark-to-market: Price changes on assets
│   ├─ Default losses: PD × LGD × EAD under scenario PDs
│   ├─ Spread widening: Duration × dSpread on fixed income
│   ├─ FX impact: Exposure × dFX on foreign portfolios
│   └─ Correlation adjustment: Higher correlation in stress
├─ Typical Stress Scenarios:
│   ├─ Recession: 5pp unemployment rise, 2-3% GDP contraction
│   ├─ Financial Crisis: Spreads 300bps, equity -40%, volatility spike
│   ├─ Rates shock: 200bps parallel shift, curve twist
│   ├─ Sector stress: Real estate -50% (real estate portfolio)
│   ├─ Counterparty: Top 3 counterparties default
│   └─ Liquidity: All illiquid assets hit with haircuts
├─ Governance:
│   ├─ Scenario approval: Board/executive review
│   ├─ Model validation: Independent review
│   ├─ Documentation: Explicit assumptions
│   ├─ Frequency: Quarterly minimum
│   └─ Escalation: Capital depleted → management action
├─ Regulatory Stress Tests:
│   ├─ Fed CCAR (US): Proprietary scenarios
│   ├─ ECB TLTRO (EU): Common scenarios
│   ├─ PRA (UK): Reverse stress test requirement
│   ├─ BIS: Macro scenarios for banks
│   └─ Frequency: Annual, integrated with capital planning
└─ Use Cases:
    ├─ Capital planning: Ensure capital survives stress
    ├─ Pricing: Add margin for stress losses to interest rate
    ├─ Risk appetite: Limits on exposures to stress scenarios
    ├─ Model validation: Check if VaR/models underestimate
    └─ Disclosure: Communication to investors/regulators
```

## 5. Mini-Project
Design and execute stress scenarios:
```python
import numpy as np
import pandas as pd
...
- **Second-order effects**: Doesn't capture feedback loops (liquidity dries up → prices fall → more defaults)

## 7. Key References
- [Basel III Stress Testing](https://www.bis.org/basel_framework/chapter/MRA/40.htm) - Regulatory framework
- [Fed CCAR/DFAST](https://www.federalreserve.gov/supervisionreg/ccar_about.htm) - US regulatory stress test
- [ECB SSM Stress Test](https://www.bankingsupervision.europa.eu/ecb/pub/stress_test/html/index.en.html) - EU framework
