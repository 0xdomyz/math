...
print(f"  True Model: Y = 2 + 3*X1 + 1.5*X2 + Îµ")
print(f"  Heteroskedasticity: Ïƒáµ¢ = 0.5*X1áµ¢ (variance increases with X1)")
...
axes[0, 2].set_title('ÎµÂ² vs X1 (Positive Relationship)')
...
print("\nBreusch-Pagan Test:")
print(f"  Hâ‚€: Homoskedasticity (constant variance)")
print(f"  LM Statistic: {bp_stat:.4f}")
print(f"  p-value: {bp_pval:.6f}")
if bp_pval < 0.05:
    print("  âœ“ Reject Hâ‚€: Heteroskedasticity detected")
else:
...
if gq_pval < 0.05:
    print("  âœ“ Reject Hâ‚€: Variance differs across X1 range")
else:
    print("  Fail to reject: No evidence of heteroskedasticity")
...
# Since we know the true variance function: Ïƒáµ¢Â² âˆ X1áµ¢
# Optimal weights: wáµ¢ = 1/Ïƒáµ¢ = 1/âˆšX1áµ¢ (assuming we know the form)
weights = 1 / df['X1']
...
axes2[1, 0].set_xlabel('True Variance (Ïƒáµ¢Â²)')
...
print(f"  OLS + HC3:           {coverage_hc3:.3f}  {'âœ“' if 0.94 <= coverage_hc3 <= 0.96 else 'âš '}")
print(f"  WLS (known weights): {coverage_wls:.3f}  {'âœ“' if 0.94 <= coverage_wls <= 0.96 else 'âš '}")
...
print("\n1. Detection:")
print("   â€¢ Visual: Fan-shaped residual plot indicates heteroskedasticity")
print("   â€¢ Formal: BP test, White test both reject homoskedasticity")

print("\n2. Consequences of Ignoring:")
print(f"   â€¢ Coefficients: Unbiased but inefficient")
print(f"   â€¢ Standard errors: Biased (OLS underestimates by ~{(ols_hc3.bse['X1'] - ols_model.bse['X1'])/ols_model.bse['X1']*100:.1f}% for X1)")
print(f"   â€¢ Coverage rate: {coverage_ols:.1%} instead of 95%")

print("\n3. Correction Methods:")
print("   â€¢ Robust SE (HC3): Always safe, valid inference, no efficiency gain")
print("   â€¢ WLS: More efficient if variance function known")
print("   â€¢ FGLS: Asymptotically efficient, may not beat robust SE in finite samples")

print("\n4. Practical Recommendations:")
print("   â€¢ Default: Use robust SE (HC3) for valid inference")
print("   â€¢ Known structure (grouped data): Use WLS")
print("   â€¢ Large N with clear pattern: Consider FGLS")
print("   â€¢ Always plot residuals before formal tests")
