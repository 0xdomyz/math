...
Another practical point is scale mismatch between statistical and economic significance. A coefficient may be highly statistically significant in large samples yet too small to matter after costs, hedging frictions, or capital charges. Conversely, an economically material effect may appear noisy in limited data but still justify action when combined with prior information and scenario analysis. Strong notes in Maximum Likelihood Estimation therefore connect estimates to decision thresholds and expected utility, not only p-values.
