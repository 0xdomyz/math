...
## 1. Concept Skeleton
**Definition:** Fundamental principles quantifying likelihood of outcomes: probability as ratio of favorable to total outcomes  
**Purpose:** Foundation for all gambling analysis, risk assessment, decision-making under uncertainty  
**Prerequisites:** Counting, set theory, basic algebra

## 2. Comparative Framing
| Concept | Probability | Odds | Expected Value |
|---------|------------|------|-----------------|
| **Definition** | P(A) = favorable/total | Ratio odds:against | E(X) = Σ(outcome × probability) |
| **Range** | 0 to 1 | 0 to ∞ (or negative) | Unbounded |
| **Use** | Likelihood measure | Betting convention | Decision metric |
| **Conversion** | P → Odds: p/(1-p) | Odds → P: o/(1+o) | Guides optimal bet |

## 3. Examples + Counterexamples

**Simple Example:**  
Fair coin: P(heads) = 1/2 = 0.5 = 50% = 1:1 odds. Fair payout: \$1 bet → \$2 return.

**Failure Case:**  
Assuming P(rain tomorrow) = P(rain next week) when weather patterns autocorrelated. Independence assumption violated.

**Edge Case:**  
Rare events (p < 0.01): Small probability differences matter enormously; P(0.001) vs P(0.002) is 2× difference.

## 4. Layer Breakdown
```
Probability Framework:
├─ Basic Rules:
│   ├─ Probability space: (Ω, σ-algebra, P) with Ω = all outcomes
│   ├─ Axioms: P(Ω)=1, P(A)≥0, P(A∪B)=P(A)+P(B) if disjoint
...
│   └─ Independence: P(A|B) = P(A) ⟹ A, B independent
...
└─ Conversion:
    ...
    ├─ American Odds: ±100 corresponds to -110 payout
    └─ Implied Probability: p = 1/(decimal_odds)
```

## 5. Mini-Project
Calculate probabilities and convert odds formats:
```python
import numpy as np
...
## 6. Challenge Round
When do probability calculations fail or mislead?
- Dependence assumed as independence (correlated events)
- Base rate neglect (ignore prior P(A) when evaluating P(A|B))
- Small number bias (variance high with small samples)
- Changing problem setup mid-analysis (p-hacking)
- Rare event probability extreme sensitivity (0.1% vs 0.2% dramatically different)
...
---
**Status:** Foundation for gambling analysis | **Complements:** House Edge, Expected Value, Combinatorics
