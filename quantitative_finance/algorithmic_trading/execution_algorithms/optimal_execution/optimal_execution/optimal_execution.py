...
np.random.seed(42)
print("Setup complete for topic: optimal execution")
...
class OptimalExecutionAlmgrenChriss(ExecutionAlgorithm):
    ...
    def get_execution_path(self):
        ...
        Solves: min E[IS] = Î³XÂ² + Ï„âˆ«x_tÂ² dt + Î»*ÏƒÂ²*âˆ«x_tÂ² dt
        """
        # Almgren-Chriss solution for linear impact
        # x_t = X * sqrt(kappa/T) * sinh(g*t) / sinh(g*T)
        # where g = sqrt(kappa) * sqrt(lambda_risk * sigmaÂ² / tau)
        ...
        def objective(x_path):
            ...
            # Permanent impact cost: Î³ Ã— (cumulative executed)Â²
            permanent_cost = self.gamma * np.sum(X_cumsum**2)
            
            # Temporary impact cost: Ï„ Ã— (sum of squared trades)
            temporary_cost = self.tau * np.sum(x_path**2)
            
            # Timing risk: Î» Ã— ÏƒÂ² Ã— (sum of squared trades) Ã— (time to completion)
...
class ExecutionCostCalculator:
    ...
    def calculate_execution_cost(self, x_path):
        ...
        IS = Î£[x_t * (temporary_impact + permanent_impact)]
        """
        n = len(x_path)
        costs = np.zeros(n)
...
algorithms = {
    ...
    'Almgren-Chriss (Î»=1e-6)': OptimalExecutionAlmgrenChriss(total_shares, T, lambda_risk=1e-6, impact_params=impact_params),
    'Almgren-Chriss (Î»=1e-5)': OptimalExecutionAlmgrenChriss(total_shares, T, lambda_risk=1e-5, impact_params=impact_params),
    'Almgren-Chriss (Î»=1e-4)': OptimalExecutionAlmgrenChriss(total_shares, T, lambda_risk=1e-4, impact_params=impact_params),
}
...
print("\n" + "="*80)
print("SENSITIVITY: RISK AVERSION PARAMETER (Î»)")
print("="*80)
...
print(f"\n{'Î»':<15} {'Cost (bps)':<15} {'Max Size':<15} {'Avg Size':<15}")
print("-" * 60)
...
# Plot 6: Sensitivity to Î»
ax = axes[1, 2]
ax.semilogx(df_sensitivity['lambda'], df_sensitivity['cost_bps'], marker='o', linewidth=2, markersize=6)
ax.set_title('Sensitivity: Execution Cost vs Risk Aversion (Î»)')
ax.set_xlabel('Risk Aversion Parameter (Î»)')
ax.set_ylabel('Cost (basis points)')
ax.grid(alpha=0.3)
...
# %%
if 'df' in globals() and 'signal' in df.columns:
    ax = df.plot(x='t', y='signal', figsize=(10, 4), title='optimal execution - Signal View')
