...
## 1. Concept Skeleton
**Definition:** Probability analysis of blackjack outcomes; dealer rules, player strategy, and card counting implications  
**Purpose:** Calculate hand probabilities, determine optimal basic strategy, evaluate card counting advantage  
**Prerequisites:** Probability, conditional probability, deck composition, basic strategy concepts

## 2. Comparative Framing
| Scenario | Player Probability | House Probability | Push | Outcome |
|----------|-------------------|------------------|------|---------|
| Blackjack (21 on 2 cards) | 4.83% | 4.83% | 0% | -0.17% player |
| Bust (over 21) | 7.54% | 16.04% | 0% | +8.5% player |
| Dealer 16 stand | Varies | 0% | Variable | Strategy-dependent |
| Hit 12-16 | Hit | Dealer hits | Varies | Soft 17 rule |

## 3. Examples + Counterexamples

**Simple Example:**  
Player has 16, dealer shows 6. Dealer busts 42% of the time; player should stand (bust risk 62% if hitting).

**Failure Case:**  
"I feel lucky, always hit 16." Ignoring probabilities; basic strategy minimizes house edge to 0.5%.

**Edge Case:**  
Soft 17 (Ace+6): Can hit without busting. Hitting soft 17 vs dealer upcard changes odds significantly.

## 4. Layer Breakdown
```
Blackjack Probability Framework:
├─ Game Setup:
│   ├─ Single deck: 52 cards, 4 of each rank
│   ├─ Standard shoe: 6-8 decks, 312-416 cards
│   ├─ Blackjack pays 3:2 (some casinos 6:5, worse)
│   ├─ Dealer hits on soft 17 (common)
│   ├─ Player wins on 21 or dealer bust
│   └─ Push on tie
├─ Hand Probabilities (Fresh shoe):
│   ├─ Natural blackjack: 4.83% (32/663)
│   ├─ Total 21: 4.83% for blackjack, plus made 21s
│   ├─ Bust probability: 16.5% for dealer, varies for player
│   ├─ Soft 17 (Ace+6): 1.29%
│   ├─ Soft 18 (Ace+7): Probability varies by hit/stand
│   └─ Hard 16 (common problem): 9.7% of hands
├─ Dealer Outcomes by Upcard:
│   ├─ Dealer 2-6 (weak): 35-42% bust rate
│   ├─ Dealer 7-Ace (strong): 11-17% bust rate
│   ├─ Soft 17 rule: Dealer hits, increases busts slightly
│   ├─ Example: Dealer 6 upcard, 42% bust if standing on 16
│   └─ Player advantage: Never bust before dealer acts
├─ Basic Strategy Decision Trees:
│   ├─ Hard hands (16 or less):
│   │   ├─ Hard 5-11: Always hit (can't bust)
│   │   ├─ Hard 12: Hit if dealer 2-3 or 7-Ace, stand on 4-6
│   │   ├─ Hard 13-16: Stand on dealer 2-6, hit on 7-Ace
│   │   └─ Hard 17+: Always stand
│   ├─ Soft hands (Ace counts as 11):
│   │   ├─ Soft 13-17: Hit on most dealer cards, strategic stands
│   │   ├─ Soft 18: Stand on 2-8, hit on 9-Ace
│   │   └─ Soft 19+: Always stand
│   ├─ Pair splitting:
│   │   ├─ Split 8s and Aces always
│   │   ├─ Split 2-7 on dealer 2-7
│   │   ├─ Never split 5s or 10s
│   │   └─ 9s: Depends on dealer upcard
│   └─ Double down: Double when expected value maximized
├─ House Edge & Strategy:
│   ├─ Optimal basic strategy: 0.5% house edge
│   ├─ Random play: 2-4% house edge
│   ├─ Following intuition: 4-5% house edge
│   ├─ 6:5 blackjack payout: +1.4% edge (avoid)
│   └─ Dealer hits soft 17: +0.2% edge
├─ Card Counting Advantage:
│   ├─ Hi-Lo system: Track high (10,J,Q,K,A) vs low (2-6)
│   │   Running count: +1 for each low, -1 for each high
│   ├─ True count: Running count / (estimated decks remaining)
│   ├─ Positive count: More high cards remain → +1-2% player advantage
│   ├─ Betting strategy: Bet more when true count high
│   ├─ Penetration: Casinos use cut card at 75% dealt
│   └─ Bet spread: Increase bets 4:1 to 8:1 on positive counts
└─ Practical Implications:
    ├─ Basic strategy memorization: 0.5% edge achievable
    ├─ Card counting difficult: Requires practice, live tracking
    ├─ Casinos ban counters: Use multiple decks, frequent shuffles
    ├─ Skill advantage small: 1-2% edge vs 0.5% baseline
    └─ Bankroll requirement: Need variance cushion
```

## 5. Mini-Project
Simulate blackjack hands and verify strategy:
```python
import numpy as np
...
## 6. Challenge Round
When does blackjack strategy break down?
- Card counting (casinos mitigate with more decks, shuffles)
- Dealer deviates from rules (collusion, cheating)
- Bankroll too small (variance swings cause ruin)
- Fatigue/mistakes (basic strategy errors accumulate)
- Rule variations (3:2 vs 6:5, soft 17 hits)

## 7. Key References
- [Wikipedia: Blackjack](https://en.wikipedia.org/wiki/Blackjack)
- [Basic Strategy Charts](https://www.blackjackappraisal.com/blackjack-basic-strategy-chart/)
- [Card Counting Basics](https://www.gamblingsites.org/casino/blackjack/card-counting/)

---
**Status:** Skill-based casino game analysis | **Complements:** House Edge, Basic Strategy, Card Counting
