...
# This notebook-style script provides a runnable mini-project for claim duration termination patterns.
...
print("Starting topic workflow: claim duration termination patterns")
...
model_df = pd.concat([df[["Duration_Days", "Event_Indicator"]], cause_dummies], axis=1)
...
for cause in causes:
    cause_vec = pd.DataFrame(
        [cause_dummies.columns == c for c in cause_dummies.columns]
    ).T
...
# %%
print("Summary complete for topic: claim duration termination patterns")
