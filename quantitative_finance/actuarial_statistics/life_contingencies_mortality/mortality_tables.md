...
## 1. Concept Skeleton
**Definition:** Age-specific probabilities qₓ (death rate) and ₚₓ (survival rate) from population experience; foundational actuarial reference  
**Purpose:** Price insurance/annuities, set reserves, project cash flows, assess population mortality patterns  
**Prerequisites:** Survival probability, life expectancy, demographic data collection, mortality law fitting

## 2. Comparative Framing
| Table Type | Cohort | Period | Model-Based |
|------------|--------|---------|-------------|
| **Data** | Follow one birth year to death | Cross-section current year | Parametric (Gompertz, Lee-Carter) |
| **Time** | 50-100 years to complete | Completed immediately | Projected beyond observation |
| **Variability** | Selection bias, period shocks | Reflects current conditions | Smoothed, extrapolated |
| **Use** | Historical validation | Pricing, reserves | Forecasts, stress testing |

## 3. Examples + Counterexamples

**Simple Example:**  
2020 US Mortality Table: qₓ ≈ 0.001 at age 40, 0.015 at age 65, 0.1 at age 85; used in all insurance pricing

**Failure Case:**  
Using 1980 table in 2024: Overstates mortality by 15-20%; premiums too high, loses market share

**Edge Case:**  
Table for smokers only: Extreme mortality ages 60+, very few reach 95; reserve assumptions critically dependent

## 4. Layer Breakdown
```
Mortality Table Structure:
├─ Data Sources:
│   ├─ Insurance claims: Company experience, highly selective
│   ├─ Government vital statistics: Total population, unselected
│   ├─ Census cohorts: Follow-up surveys with demographics
│   └─ Combination: Adjust insurance data by population factors
├─ Construction:
│   ├─ 1. Count deaths dₓ by age x
│   ├─ 2. Calculate exposures Eₓ (years at risk)
│   ├─ 3. Compute qₓ = dₓ / Eₓ (death rate)
│   ├─ 4. Smooth via graduation (reduce random fluctuation)
│   └─ 5. Interpolate fractional ages (Karup-King, Beers)
├─ Graduation Methods:
│   ├─ Parametric: Fit Gompertz, Makeham, Weibull curves
│   ├─ Graphical: Draw smooth curve through plotted points
│   ├─ Whittaker-Henderson: Penalized likelihood (smoothness vs fit)
│   ├─ Spline: Piecewise polynomials with continuity constraints
│   └─ Non-parametric: Kernel smoothing, local regression
├─ Adjustments:
│   ├─ Company experience: Apply credibility weighting
│   ├─ Select period: First-year policies differ from renewal
│   ├─ Ultimate: Long-term stable mortality (age 60+)
│   ├─ Gender/Smoker: Separate tables for subgroups
│   └─ Improvement: Apply trend factors to date forward
└─ Ultimate vs Select:
    ├─ Select qₓ (attained age x, duration d): qₓ^[d]
    ├─ Ultimate qₓ (attained age x only): qₓ (after ~25 years)
    └─ "select and ultimate" table gives transition
```

**Interaction:** Raw claims → Calculate exposure-weighted rates → Smooth → Stratify → Publish

## 5. Mini-Project
Construct mortality table from claims data, graduate, and compare to standard:
```python
import numpy as np
import pandas as pd
...
## 6. Challenge Round
When mortality tables mislead:
- **Stale data**: 10-year-old table understates current mortality improvements by 10-15%
- **Mismatch**: Using general population table for highly selected insurance cohort; 20-30% error
- **Small samples**: Ages 95+ with <100 deaths; credibility-weight to standard, not relying on crude rates
- **Select/ultimate confusion**: First-year policies (select) differ from renewal (ultimate); mixing tables causes errors
- **Extreme ages extrapolation**: Table ends at 100, need projection to 120; Gompertz fits poorly at tail
- **Pandemic shock**: 2020-2021 rates spike unpredictably; static table useless, must regrade quickly

## 7. Key References
- [Human Mortality Database (HMD)](https://www.mortality.org/) - International standardized tables
- [CMI Continuous Mortality Investigation](https://www.cmi.ac.uk/) - UK insurance experience
- [Society of Actuaries Experience Studies](https://www.soa.org/research/) - US insurance data and tables

---
**Status:** Actuarial workhorse | **Complements:** Force of Mortality, Survival Probability, Life Expectancy
