...
## 1. Concept Skeleton
**Definition:** Forward-looking estimate of average credit loss; EL = PD × LGD × EAD × (1 - Correlation adjustment)  
**Purpose:** Quantify average credit risk, price credit products, provision reserves, calculate economic capital  
**Prerequisites:** Probability, expected value, PD/LGD/EAD components, portfolio risk
...
**Simple Example:**  
Loan: PD=2%, LGD=40%, EAD=$50K. EL = 0.02 × 0.40 × $50K = $400. Charge $400 to reserves annually

**Failure Case:**  
Using loan-level EL without portfolio correlation. Portfolio EL ≠ sum of individual ELs; must account for default clustering

**Edge Case:**  
EL=0 for government bond doesn't mean zero loss. Tail risk remains; expected loss ignores fat-tail probability of default
...
├─ Basic Formula:
│   └─ EL = PD × LGD × EAD
├─ Portfolio EL (with correlation):
│   ├─ EL_portfolio = Σᵢ ELᵢ [independent]
│   ├─ EL_portfolio < Σᵢ ELᵢ [negative correlation]
│   └─ EL_portfolio > Σᵢ ELᵢ [positive correlation]
├─ EL Components Interaction:
│   ├─ PD effect: ↑PD → ↑EL (linear)
│   ├─ LGD effect: ↑LGD → ↑EL (linear)
│   ├─ EAD effect: ↑EAD → ↑EL (linear)
│   └─ Correlation effect: ↑Correlation → ↑Portfolio EL
├─ Practical Calculation:
│   ├─ Loan-level: Simple multiplication
│   ├─ Portfolio: Sum across loans (accounts for correlation)
│   ├─ Multi-period: Cumulative over loan life
│   └─ Present value: Discount future losses to today
├─ Adjustments to EL:
│   ├─ Recovery timing: PV of recovery costs
│   ├─ Accrual accounting (IFRS 9/CECL): Expected credit losses
│   ├─ Forward guidance: Economic outlook adjustment
│   └─ Conservatism buffer: Add margin for model error
└─ Uses of EL Estimates:
    ├─ Loan pricing: Set interest spread to cover EL + costs
    ├─ Provisioning: P&L charge for estimated losses
    ├─ Capital allocation: Economic capital = UL + margin
    └─ Risk-adjusted returns: Profitability after risk
```

## 5. Mini-Project
Calculate and analyze EL across portfolio:
```python
import numpy as np
import pandas as pd
...
- **Correlation instability**: PD/LGD correlation changes with economic cycle; historical correlations break
- **Tail risk**: EL misses extreme losses (e.g., 99.9% VaR >> EL in crisis scenarios)
- **Model risk**: Small changes in PD assumption (1.5% → 2%) can double EL
- **Portfolio heterogeneity**: Single correlation assumption oversimplifies multi-segment portfolio
- **Forward guidance uncertainty**: Macro adjustments (recession probability) highly subjective

## 7. Key References
- [Expected Loss Formula](https://www.bis.org/basel_framework/chapter/CRE/20.htm) - Basel III standard definition
- [Vasicek Model](https://en.wikipedia.org/wiki/Vasicek_model) - Portfolio correlation framework
- [IFRS 9 Expected Credit Loss](https://www.ifrs.org/issued-standards/list-of-standards/ifrs-9-financial-instruments/) - Accounting standard for EL provisioning
