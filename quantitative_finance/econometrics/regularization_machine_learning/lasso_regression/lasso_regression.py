...
print(f"  RÂ²: {train_r2_lasso:.4f}")

print(f"\nOut-of-Sample Performance:")
print(f"  MSE: {test_mse_lasso:.4f}")
print(f"  RÂ²: {test_r2_lasso:.4f}")
...
if n_tp > 0:
    print(f"\nâœ“ Correctly Identified: {true_positives}")
if n_fp > 0:
    print(f"âœ— False Positives: {false_positives}")
if n_fn > 0:
    print(f"âœ— Missed Variables: {false_negatives}")
...
print(f"Path computed over {len(alphas_path)} Î» values")
print(f"Optimal Î» (test MSE): {alphas_path[optimal_idx]:.4f}")
print(f"Optimal Î» (CV): {optimal_lambda:.4f}")

# ===== Lambda Max Calculation =====
lambda_max_theoretical = np.max(np.abs(X_train_scaled.T @ Y_train_centered)) / train_size
print(f"\nÎ»_max (theoretical): {lambda_max_theoretical:.4f}")
print(f"  All coefficients zero for Î» â‰¥ Î»_max")
...
print(f"\nStable âˆ© True: {len(stable_true)} variables")
...
print(f"Î»_min: {lambda_min:.4f} (minimizes CV error)")
print(f"Î»_1se: {lambda_1se:.4f} (most regularized within 1 SE)")
...
print(f"\nÎ»_1se selects {n_selected - n_selected_1se} fewer variables")
print("  â†’ More parsimonious model with similar performance")
...
print(f"   Lasso Test MSE: {test_mse_lasso:.4f}, RÂ²: {test_r2_lasso:.4f}")
print(f"   Ridge Test MSE: {test_mse_ridge:.4f}, RÂ²: {test_r2_ridge:.4f}")
if test_mse_lasso < test_mse_ridge:
    print("   âœ“ Lasso achieves better prediction via sparsity")
else:
    print("   âœ— Ridge slightly better (possibly sparse structure not strong)")
...
print("\n4. Parsimony:")
print(f"   Î»_min: {n_selected} variables")
print(f"   Î»_1se: {n_selected_1se} variables (more parsimonious)")
print(f"   Consider Î»_1se for simpler, more interpretable model")

print("\n5. When to Use Lasso:")
print("   â€¢ True model sparse (few relevant predictors)")
print("   â€¢ Want interpretable variable selection")
print("   â€¢ High-dimensional (p > n or p â‰ˆ n)")
print("   â€¢ Feature screening before further analysis")

print("\n6. Limitations:")
print("   â€¢ Arbitrary selection among correlated predictors")
print("   â€¢ Biased coefficient estimates (especially for large true Î²)")
print("   â€¢ Post-selection inference requires special methods")
print("   â€¢ Consider elastic net if grouped variables important")

print("\n7. Practical Tips:")
print("   â€¢ Standardize predictors always")
print("   â€¢ Use Î»_1se for parsimony")
print("   â€¢ Bootstrap for stability assessment")
print("   â€¢ Debiased lasso for valid inference")
print("   â€¢ Nested CV for unbiased performance evaluation")
