...
## 1. Concept Skeleton
**Definition:** Present/future value factors for fixed-term payments with certainty (no mortality); foundational building blocks for actuarial valuation  
**Purpose:** Price annuities, calculate retirement income present value, value pension obligations, construct complex cashflow patterns  
**Prerequisites:** Discount factor, accumulation factor, geometric series, present value concepts

## 2. Comparative Framing
| Annuity Type | aₙ̄\| (Imm) | äₙ̄\| (Due) | sₙ̄\| (Accum) | s̈ₙ̄\| (Acc-Due) |
|--------------|------------|-----------|-------------|-----------------|
| **Timing** | End of period | Start of period | End of period | Start of period |
| **PV factor** | [1-v^n]/i | [1-v^n]/i·(1+i) | — | — |
| **FV factor** | — | — | [(1+i)^n-1]/i | [(1+i)^n-1]/i·(1+i) |
| **Formula** | ∑v^k, k=1 to n | ∑v^k, k=0 to n-1 | ∑(1+i)^k, k=0 to n-1 | ∑(1+i)^k, k=1 to n |
| **Example (n=10, i=5%)** | 7.722 | 8.108 | 12.578 | 13.207 |

## 3. Examples + Counterexamples

**Simple Example:**  
$500/month for 20 years at 4% annual (mod. to semi): a₄₀̄| × 500 × 2 ≈ 18.256 × 1000 ≈ $18,256 present value

**Failure Case:**  
Using perpetuity formula (1/i = 1/0.05 = 20) for 10-year annuity: Overstates by factor of (20/7.72) ≈ 2.6×; ignores finiteness

**Edge Case:**  
Very long term (n=100): aₙ̄| approaches 1/i (perpetuity); small difference from perpetuity formula but still material for pricing

## 4. Layer Breakdown
```
Annuity-Certain Functions Structure:
├─ Definition & Formulas:
│   ├─ aₙ̄| (annuity-immediate): PV = [1 - v^n]/i = ∑ₖ₌₁ⁿ v^k
│   ├─ äₙ̄| (annuity-due): PV = [1 - v^n]/i · (1+i) = ∑ₖ₌₀^{n-1} v^k
│   ├─ sₙ̄| (accumulation-immediate): FV = [(1+i)^n - 1]/i
│   ├─ s̈ₙ̄| (accumulation-due): FV = [(1+i)^n - 1]/i · (1+i)
│   └─ Perpetuity-immediate: a̅_∞ = 1/i
├─ Relationships:
│   ├─ äₙ̄| = (1+i) · aₙ̄|  (due is one period forward)
│   ├─ s̈ₙ̄| = (1+i) · sₙ̄|
│   ├─ aₙ̄| = 1 - v^n · aₙ̄| / (1+i)  (mixed formula)
│   ├─ sₙ̄| · v^n = aₙ̄|  (accumulate back discounts)
│   └─ äₙ̄| = 1 + aₙ̄|  (first payment today + remaining)
├─ Special Cases:
│   ├─ n=1: a₁̄| = v, ä₁̄| = 1
│   ├─ i→0: aₙ̄| → n (simple addition)
│   ├─ n→∞: aₙ̄| → 1/i (perpetuity)
│   └─ Deferred: ₘ|aₙ̄| = v^m · aₙ̄| (start m years out)
├─ Spreadsheet Implementations:
│   ├─ Using PV function: PV(i, n, -PMT)
│   ├─ Using FV function: FV(i, n, -PMT)
│   ├─ Adjusted for due: Multiply by (1+i)
│   └─ Deferred: Multiply PV by v^m
└─ Actuarial Extensions:
    ├─ Life annuity: aₓ = ∑ₖ₌₀^∞ v^k · ₖpₓ  (adds mortality)
    ├─ Temporary annuity: aₓ:n̄| = ∑ₖ₌₀^{n-1} v^k · ₖpₓ
    ├─ Increasing annuity: (Ia)ₙ̄| = ∑ₖ₌₁ⁿ k·v^k
    └─ Stepped annuity: Payout changes by age/year
```

**Interaction:** Specify PMT, n, i → Select aₙ̄| or äₙ̄| → PV = PMT × factor → Apply mortality if life-contingent

## 5. Mini-Project
Calculate annuity factors, price retirement income, and value pension liabilities:
```python
import numpy as np
import pandas as pd
...
## 6. Challenge Round
When annuity factors mislead:
- **No mortality assumption**: Annuity-certain assumes everyone survives; life annuities must weight by ₖpₓ for realistic pricing
- **Fixed rate assumption**: aₙ̄| assumes constant i; variable-rate products require interest rate scenario modeling
- **Frequency mismatch**: Annual factor applied to monthly payments causes systematic errors; convert rates first
- **Long-tail uncertainty**: 50+ year horizons require scenario analysis; point estimate misleading
- **Basis risk**: Using government bond rates when actual returns differ; creates systematic mismatch between assumption and reality

## 7. Key References
- [Annuity (Wikipedia)](https://en.wikipedia.org/wiki/Annuity_(finance)) - Definitions and formulas
- [Bowers et al., Actuarial Mathematics (Chapter 2)](https://www.soa.org/) - Complete annuity derivations
- [SOA Exam FM Study Manual](https://www.soa.org/education/exam-req/edu-exam-fm-detail.aspx) - Practice problems and solutions

---
**Status:** Foundational pricing tool | **Complements:** Life Annuities, Pension Valuation, Bond Pricing
