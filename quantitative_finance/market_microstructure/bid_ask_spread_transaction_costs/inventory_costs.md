...
## 1. Concept Skeleton
**Definition:** Risk premium market makers charge for holding unwanted positions, compensating for price volatility exposure  
**Purpose:** Explains spread component arising from dealer risk-bearing, drives inventory management strategies  
**Prerequisites:** Market maker models, portfolio risk, mean-variance optimization, bid-ask spread decomposition

## 2. Comparative Framing
| Cost Type | Inventory Cost | Adverse Selection | Order Processing | Opportunity Cost |
|-----------|----------------|-------------------|------------------|------------------|
| **Risk Source** | Price volatility | Information asymmetry | Fixed overhead | Capital tied up |
| **Duration** | Holding period | Permanent | Per transaction | Position lifetime |
| **Mitigation** | Adjust quotes, hedge | Widen spread | Economies of scale | Faster turnover |
| **% of Spread** | 10-30% typical | 40-70% | 10-20% | Varies |

## 3. Examples + Counterexamples

**Simple Example:**  
Dealer buys 1000 shares at $100, holds overnight. Stock volatility σ=2%/day → inventory risk = $2,000 (1σ move)

**Hedged Position:**  
Dealer sells stock, immediately hedges with index futures → inventory risk minimized, inventory cost near zero

**Edge Case:**  
Flash crash: Dealer accumulates large inventory as liquidity provider, price drops 10% in minutes → massive inventory loss

## 4. Layer Breakdown
```
Inventory Cost Structure:
├─ Risk Components:
│   ├─ Price Risk: σ² × Position Size × Time
│   ├─ Directional Risk: Unhedged beta exposure
│   ├─ Volatility Risk: Gamma exposure from options
│   └─ Liquidity Risk: Cannot exit position quickly
├─ Stoll (1978) Model:
│   ├─ Risk-Averse Dealer: Utility function U(W) with CARA
│   ├─ Optimal Spread: S = 2α × σ² × Q
│   │   α = risk aversion coefficient
│   │   σ = volatility, Q = position size
│   └─ Inventory Adjustment: Bid/ask skewed to flatten position
├─ Inventory Management:
│   ├─ Mean Reversion Strategy: Quotes favor inventory reduction
│   ├─ Hedging: Use correlated assets, options, futures
│   ├─ Position Limits: Maximum inventory constraints
│   └─ Dynamic Pricing: Widen spread when inventory extremes
├─ Avellaneda-Stoikov Model (2008):
│   ├─ Reservation Price: r(t) = S - q × γ × σ² × (T-t)
│   │   S = fair price, q = inventory, γ = risk aversion
│   ├─ Optimal Quotes: δ^bid/ask = (1/γ) × ln(1 + γ/k) ± inventory skew
│   └─ Inventory Penalty: Exponential utility maximization
└─ Empirical Patterns:
    ├─ Quote Skewing: Positive inventory → lower ask (eager to sell)
    ├─ Spread Widening: Extreme inventory → wider spreads
    ├─ Intraday Cycle: Flatten at close to avoid overnight risk
    └─ Correlation with Volatility: Higher σ → higher inventory cost
```

**Interaction:** Trade → inventory deviation → quote adjustment → mean reversion → inventory balanced

## 5. Mini-Project
Simulate market maker inventory management and optimal quoting:
```python
import numpy as np
...
## 6. Challenge Round
How does inventory risk differ across market conditions and assets?
- **Volatility scaling**: Inventory cost ∝ σ², so VIX spike from 15 to 30 → 4x inventory cost (dealers widen spreads during volatility)
- **Liquidity constraints**: Illiquid stocks harder to hedge → higher inventory costs → wider spreads (small-cap vs large-cap spread differential)
- **Holding period**: Intraday dealers flatten positions at close to avoid overnight risk (closing auction activity spikes)
- **Correlation structure**: Correlated inventory across assets amplifies risk (market maker in SPY, QQQ, IWM has correlated exposure)
- **Hedging availability**: Options, futures reduce inventory costs → tighter spreads for assets with liquid derivatives markets

## 7. Key References
- [Stoll (1978) - Supply of Dealer Services in Securities Markets](https://www.jstor.org/stable/2327007)
...
- [Hendershott & Menkveld (2014) - Price Pressures](https://onlinelibrary.wiley.com/doi/abs/10.1111/jofi.12498)
- [Comerton-Forde et al (2010) - Time Variation in Liquidity](https://www.sciencedirect.com/science/article/abs/pii/S1042443110000131)

---
**Status:** Market maker risk management driver | **Complements:** Bid-Ask Spread, Adverse Selection, Optimal Market Making
