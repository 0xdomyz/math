...
## 1. Concept Skeleton
**Definition:** Instrumental variable estimation via two sequential OLS regressions to handle endogeneity  
**Purpose:** Obtain consistent parameter estimates by isolating exogenous variation using instruments  
**Prerequisites:** Instrumental variables theory, OLS mechanics, matrix algebra, asymptotic theory

## 2. Comparative Framing
| Method | 2SLS | OLS | GMM | Limited Info ML |
|--------|------|-----|-----|-----------------|
| **Stages** | Two OLS regressions | Single regression | Moment conditions | Simultaneous system |
| **Instruments** | Required | Not applicable | Flexible (overidentified) | Full system |
| **Efficiency** | Efficient (just-identified) | Most efficient if valid | Most efficient (optimal weighting) | Full info efficient |
| **SE Adjustment** | Required for two stages | Standard | Robust sandwich | Model-specific |

## 3. Examples + Counterexamples

**Classic Example:**  
Supply-demand simultaneity: Price endogenous in demand equation. Instrument: Weather shock affects supply (shifts curve) but not demand preferences.

**Failure Case:**  
Using predicted values from first stage but forgetting to adjust standard errors → artificially precise estimates, invalid inference.

**Edge Case:**  
Exact multicollinearity in first stage: Multiple instruments perfectly correlated. Coefficient matrix singular, estimation fails.

## 4. Layer Breakdown
```
Two-Stage Least Squares Procedure:
├─ Setup:
│   ├─ Structural Equation: Y = Xβ + ε, E[X'ε] ≠ 0
│   ├─ Instruments: Z (exogenous), dim(Z) ≥ dim(X_endog)
│   └─ Partition: X = [X_exog, X_endog]
├─ Stage 1: Project endogenous variables onto instrument space
│   ├─ Regression: X_endog = Π₀ + ΠZ + ν
│   │   └─ OLS: Π̂ = (Z'Z)⁻¹Z'X_endog
│   ├─ Predicted: X̂_endog = Z Π̂
│   └─ Diagnostics:
│       ├─ F-test: H₀: Π = 0 (weak instrument if F < 10)
│       └─ R²: Proportion of variation explained by Z
├─ Stage 2: Regress outcome on predicted endogenous variables
│   ├─ Regression: Y = X̂_endog β + X_exog γ + residual
│   │   └─ OLS on (X̂, X_exog): β̂_2SLS
│   └─ Matrix Form: β̂_2SLS = [X̂'X̂]⁻¹X̂'Y
│       where X̂ = [X̂_endog, X_exog]
├─ Standard Errors:
│   ├─ Naive (wrong): Use stage 2 SE → underestimates uncertainty
│   ├─ Correct: Adjust for estimated X̂_endog
│   │   └─ Var(β̂) = σ²(X'P_Z X)⁻¹, P_Z = Z(Z'Z)⁻¹Z'
│   └─ Robust: Sandwich estimator for heteroscedasticity
└─ Identification:
    ├─ Rank Condition: rank(E[Z'X]) = k (full column rank)
    └─ Order Condition: # instruments ≥ # endogenous variables
```

**Interaction:** Stage 1 purges endogenous variation → Stage 2 uses cleaned variation for consistent estimation

## 5. Mini-Project
Implement 2SLS manually and compare with packaged estimators:
```python
import numpy as np
import pandas as pd
...
# X₂ is endogenous, instruments Z₁ and Z₂
...
# Outcome (true coefficients: β₁=1, β₂=2)
...
print(f"R²: {stage1_model.rsquared:.4f}")
print("\nCoefficients:")
...
if f_stat_instruments < 10:
    ...
    print("✓ Instruments appear strong")
...
if len(['Z1', 'Z2']) > 1:  # Multiple instruments
    ...
    print(f"H₀: All instruments are valid (exogenous)")
    print(f"Sargan statistic: {sargan_stat:.4f}")
    print(f"Degrees of freedom: {df_sargan}")
    print(f"p-value: {p_value:.4f}")
    
    if p_value < 0.05:
        print("✗ Reject H₀: At least one instrument appears invalid")
    else:
        print("✓ Fail to reject: Instruments appear valid")
```

## 6. Challenge Round
Common 2SLS pitfalls and diagnostics:
- **Forgetting SE correction**: Using stage 2 OLS standard errors underestimates uncertainty by ~20-50%
- **Weak instruments**: If first stage F < 10, finite sample bias toward OLS persists
- **Testing endogeneity**: Add stage 1 residuals to OLS; significant coefficient confirms endogeneity
- **Many weak instruments**: Better to use few strong instruments than many weak ones (bias amplification)
- **Exactly identifying**: Use all instruments in stage 1, not just "significant" ones (pre-test bias)

## 7. Key References
- [Wooldridge - Econometric Analysis of Cross Section and Panel Data (Ch 5)](https://mitpress.mit.edu/9780262232586/econometric-analysis-of-cross-section-and-panel-data/)
- [Stock & Yogo (2005) - Testing for Weak Instruments](https://www.nber.org/papers/w0284)
- [Baum et al. (2007) - Enhanced routines for IV/GMM estimation](https://www.stata-journal.com/article.html?article=st0030_3)

---
**Status:** Standard IV implementation | **Complements:** Instrumental Variables, GMM, LIML, Fuller-k
