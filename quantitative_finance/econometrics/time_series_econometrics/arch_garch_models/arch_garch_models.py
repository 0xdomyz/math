...
# GARCH(1,1) process: ÏƒÂ²â‚œ = 0.00001 + 0.08Â·ÎµÂ²â‚œâ‚‹â‚ + 0.90Â·ÏƒÂ²â‚œâ‚‹â‚
omega = 0.00001
alpha = 0.08
beta = 0.90
...
print(f"  Persistence (Î±+Î²): {alpha_est + beta_est:.6f}")
...
for t in range(1, forecast_steps):
    # Multi-step forecast: E[ÏƒÂ²â‚œâ‚Šâ‚–] converges to unconditional variance
...
print(f"""
Key GARCH Results:
1. Persistence (Î±+Î² = {alpha_est + beta_est:.4f}):
   â†’ Volatility very persistent (close to 1)
   â†’ Shocks take {half_life:.0f} days to half-dissipate
   
2. VaR Estimates:
   â†’ 95% confidence: Maximum daily loss = {var_estimates[0.95]:.3f}%
   â†’ 99% confidence: Maximum daily loss = {var_estimates[0.99]:.3f}%
   
3. Conditional volatility changes over time:
   â†’ Clustering: High volatility followed by high volatility
   â†’ Mean reversion: Extreme moves revert toward average
   
4. Forecast implications:
   â†’ Near-term volatility: {sigma_forecast[0]:.6f}
   â†’ Term structure: Vol mean reverts to long-run {np.sqrt(sigma2_terminal):.6f}
""")
