...
Calculate and identify positive EV opportunities in gambling.
"""
...
axes[0, 0].fill_between(
    probs * 100, evs_fair, 0, where=(np.array(evs_fair) > 0), alpha=0.2, color="green"
)
axes[0, 0].fill_between(
    probs * 100, evs_fair, 0, where=(np.array(evs_fair) <= 0), alpha=0.2, color="red"
)
...
axes[1, 0].plot(
    num_bets_range, ev_cumulative, linewidth=2, label="Expected Value", color="green"
)
