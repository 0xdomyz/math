...
# This notebook-style script provides a runnable mini-project for survival probability.
...
print("Starting topic workflow: survival probability")
...
# Source: survival_probability.md
...
qx_base = np.array(
    ...
    + list(np.linspace(0.00295, 0.05, 96))
)  # Extend to age 120
...
print(
    life_table[life_table["Age"].isin([0, 20, 40, 60, 80, 100])].to_string(index=False)
)
print()
...
for age in [20, 40, 60, 80]:
    print(
        f"{age}\t{px_matrix[age, 1]:.4f}\t{px_matrix[age, 5]:.4f}\t"
...
ax.set_title("Survival Probability Heatmap (p)", fontsize=12, fontweight="bold")
...
# %%
print("Summary complete for topic: survival probability")
