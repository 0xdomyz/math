"""
Independence and Dependence: Execution and visualizations.
...
import numpy as np
import pandas as pd
from independence_dependence_models import PokerDeck, Roulette
...
def run():
    ...
    for _ in range(n_simulations):
        ...
        outcomes_dependence.append(
            {"high_dealt": n_high_dealt, "p_high_given": p_high_given}
        )
    ...
    print("   Negative correlation = dependence ✓ (more high dealt → fewer remain)")
    ...
    p = np.poly1d(z)
    ax2.plot(
        sorted(high_dealt_list),
        p(sorted(high_dealt_list)),
        "r--",
        linewidth=2,
        alpha=0.8,
    )
    ...
    ax3 = axes[1, 0]
    ax3_twin = ax3.twinx()
    ax3.bar(df_corr["game"], df_corr["bet"], alpha=0.5, color="blue", label="Bet Size")
    ax3_twin.plot(
        df_corr["game"],
        df_corr["cumulative_loss"],
        "o-",
        color="purple",
        linewidth=2,
        markersize=4,
    ...
    for n_spin in n_values:
        ...
        roulette_test_results.append(
            {"n_spins": n_spin, "p_value": result["p_value"], "chi2": result["chi2"]}
        )
    ...
    ax4.axhline(
        y=0.05, color="red", linestyle="--", linewidth=2, label="α = 0.05 threshold"
    )
