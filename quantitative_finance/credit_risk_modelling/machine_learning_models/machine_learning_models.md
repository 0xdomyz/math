...
├─ Data Preparation:
│   ├─ Features: Raw + engineered variables
│   ├─ Targets: Binary (default/non-default) or multi-class
│   ├─ Imbalance handling: SMOTE, class weights, or stratified sampling
│   └─ Scaling: Normalization for distance-based algorithms
├─ Model Classes:
│   ├─ Tree-based:
│   │   ├─ Random Forest: Ensemble of bootstrap samples
│   │   ├─ Gradient Boosting: Sequential error correction
│   │   ├─ XGBoost: Regularized boosting
│   │   └─ LightGBM: Fast large-scale boosting
│   ├─ Linear/Logistic:
...
│   │   ├─ CNN: Convolutional (image data)
│   │   └─ RNN: Sequential (time series)
...
│   ├─ Walk-forward: Time-series appropriate
│   └─ Stratified: Preserve class imbalance in folds
├─ Evaluation Metrics:
│   ├─ Discrimination: AUC-ROC, Gini, K-S statistic
│   ├─ Calibration: Actual vs predicted default rate
│   ├─ Stability: Robustness across time periods
│   ├─ Fairness: Equal treatment across demographics
│   └─ Explainability: SHAP values, feature importance
├─ Monitoring in Production:
│   ├─ Model drift: Shift in predicted vs actual PD
│   ├─ Population drift: Input distribution changes
│   ├─ Performance degradation: Declining AUC
│   └─ Retraining triggers: Monthly/quarterly refresh
└─ Challenges:
    ├─ Black-box risk: Difficulty explaining decisions
    ├─ Data quality: Garbage in → garbage out
    ├─ Overfitting: High train accuracy, low test
    ├─ Regulatory concerns: Fair lending, bias
    └─ Concept drift: Relationships change over time
```

## 5. Mini-Project
Build and compare ML models for credit risk:
```python
import numpy as np
import pandas as pd
...
## 7. Key References
- [Scikit-Learn Credit Scoring](https://scikit-learn.org/stable/modules/ensemble.html) - Random Forest, Gradient Boosting implementations
- [XGBoost for Credit Risk](https://xgboost.readthedocs.io/en/stable/) - Production-grade gradient boosting
- [ML Fairness in Finance](https://arxiv.org/abs/1908.04913) - Bias detection and mitigation strategies
