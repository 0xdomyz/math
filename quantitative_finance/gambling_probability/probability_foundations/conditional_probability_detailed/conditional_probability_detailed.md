...
## 1. Concept Skeleton
**Definition:** Probability of event A occurring given that event B has already occurred: P(A|B)  
**Purpose:** Model sequential games, card counting scenarios, and dependent outcomes  
**Prerequisites:** Basic probability, independence concept

## 2. Comparative Framing
| Concept | Conditional P(A\|B) | Independent P(A) | Joint P(A,B) | Bayesian Update |
|---------|---------------------|-----------------|------------|-----------------|
| **Formula** | P(A∩B) / P(B) | P(A) | P(A) × P(B) if independent | P(B\|A) × P(A) / P(B) |
| **Use Case** | Draw without replacement | Coin flips | Both events occur | Update belief with new info |
| **Gambling Application** | Poker odds after flop | Roulette spin | Multi-card combos | Card counting |
| **Dependence** | Dependent outcomes | No dependency | Varies | Updates based on evidence |

## 3. Examples + Counterexamples

**Example (Card Draw):**  
P(second ace | drew first ace) = 3/51 ≈ 5.9% (not 4/52 ≈ 7.7%)

**Example (Poker):**  
P(flush | 4 same suit on flop) depends on remaining deck composition.

**Counterexample (Independence):**  
P(heads on flip 2 | heads on flip 1) = 0.5 (coin flips are independent)
...
│  └─ Law of total probability: Condition on partition
├─ Poker Applications:
│  ├─ Post-flop equity: Given community cards, hand equity
│  ├─ Pot odds: Compare conditional win rate to payout
│  └─ Outs calculation: Cards improving hand, conditional on board
├─ Card Counting:
│  ├─ Running count: Adjusted by cards dealt
│  ├─ True count: Adjust for remaining deck
│  └─ Probability shift: Conditional on penetration
├─ Dependence vs Independence:
│  ├─ Dependent: P(A|B) ≠ P(A) (cards without replacement)
│  ├─ Independent: P(A|B) = P(A) (coin flips)
│  └─ Testing: Check if P(A∩B) = P(A) × P(B)
├─ Bayesian Updates:
│  ├─ Prior: P(A) before observation
│  ├─ Likelihood: P(observation|A)
│  ├─ Posterior: P(A|observation) updated
│  └─ Application: Sequential belief updating in tournaments
└─ Gaming Examples:
   ├─ Blackjack: P(bust|hit on 15) depends on deck composition
   ├─ Poker: P(win|all-in on river) depends on hand vs range
   └─ Roulette: Each spin independent, P(red|red streak) unchanged
```

**Interaction:** Observe event B → update conditional probability of A → adjust strategy accordingly.

## 5. Mini-Project
Compute conditional probabilities for card games:
```python
import numpy as np
...
## 6. Challenge Round
Why does conditioning on prior draws matter in card games but not in independent games like roulette?
- Drawing cards without replacement changes deck composition → conditional probabilities shift
- Roulette wheel is reset each spin → outcomes remain independent
- Dependence is structural: sequential card draws without replacement
- Independence is structural: random wheel spin unaffected by history

## 7. Key References
- [Conditional Probability (Wikipedia)](https://en.wikipedia.org/wiki/Conditional_probability)
- [Bayes' Theorem](https://en.wikipedia.org/wiki/Bayes%27_theorem)
- [Card Counting (Wizard of Odds)](https://www.wizardofodds.com/games/blackjack/card-counting/)

---
**Status:** Foundational probability concept | **Complements:** Independence & Dependence, Bayesian Updating, Poker Probabilities
