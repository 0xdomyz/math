...
# True model: log(wage) = Î²â‚€ + Î²â‚(education) + Î²â‚‚(ability) + Îµ
...
if first_stage.fvalue < 10:
    print("âš ï¸  WARNING: Weak instrument detected!")
else:
    print("âœ“ Instrument appears strong")
print(f"RÂ² (First Stage): {first_stage.rsquared:.4f}\n")
...
summary_text = f"""
OLS (Naive):
  â€¢ Bias: {ols_model.params['education'] - true_return:+.4f}
  â€¢ SE: {ols_model.bse['education']:.4f}
  â€¢ Consistent: No (omitted variable)

IV/2SLS:
  â€¢ Bias: {iv_model.params['education'] - true_return:+.4f}
  â€¢ SE: {iv_model.std_errors['education']:.4f}
  â€¢ Consistent: Yes (if valid instrument)
  â€¢ Efficiency Loss: {se_ratio:.1f}x

Oracle (ability included):
  â€¢ Bias: {oracle_model.params['education'] - true_return:+.4f}
  â€¢ SE: {oracle_model.bse['education']:.4f}
  â€¢ Best case (unobservables known)
"""
...
if hausman_model.pvalues['resid'] < 0.05:
    print("âœ“ Reject Hâ‚€: Education is endogenous (use IV)")
else:
