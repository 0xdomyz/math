...
# This notebook-style script provides a runnable mini-project for force of mortality.
...
print("Starting topic workflow: force of mortality")
...
result_gm = minimize(
    negative_likelihood_gm, p0, args=(ages_fit, dx_fit, lx_fit), method="Nelder-Mead"
)
...
p0_g = [0.0002, 1.07]
result_g = minimize(
    negative_likelihood_g, p0_g, args=(ages_fit, dx_fit, lx_fit), method="Nelder-Mead"
)
...
def kernel_smooth_mu(ages, mu_empirical, bandwidth=3):
    ...
    base_ages = ages[: len(mu_empirical)]

    for i, x in enumerate(ages):
        # Epanechnikov kernel: K(u) = 0.75(1 - u2) for |u|  1
        distances = np.abs(base_ages - x)
        ...
        if weight_sum > 0:
            kernel_weights = kernel_weights / weight_sum  # Normalize
...
ax.set_ylabel("Force of Mortality 14", fontsize=11)
...
ax.semilogy(
    ages_empirical, mu_empirical, "o", markersize=4, alpha=0.5, label="Empirical"
)
ax.semilogy(ages, mu_gm_fitted, "r-", linewidth=2, label="Gompertz-Makeham")
ax.semilogy(ages, mu_kernel, "g--", linewidth=2, label="Kernel")
ax.set_xlabel("Age (x)", fontsize=11)
ax.set_ylabel("Force of Mortality 14 (log scale)", fontsize=11)
...
# Plot 4: Survival probability (derived from 14)
ax = axes[1, 1]
...
ax.plot(ages, Sx_gm, "r-", linewidth=2, label="From GM fitted 14")
ax.plot(ages, Sx_kernel, "g--", linewidth=2, label="From kernel 14")
ax.set_xlabel("Age (x)", fontsize=11)
ax.set_ylabel("Survival Probability S", fontsize=11)
...
print("Age\tTrue 14\tEstimated 14\tError")
print("-" * 50)
for age in [30, 50, 70, 90]:
    idx = age
    print(
        f"{age}\t{mu_true[idx]:.5f}\t{mu_gm_fitted[idx]:.5f}\t{abs(mu_true[idx] - mu_gm_fitted[idx]):.5f}"
    )
...
# %%
print("Summary complete for topic: force of mortality")
