...
## 1. Concept Skeleton
**Definition:** Aₓ^(I) = ∑ₖ₌₁^∞ k·v^k·ₖ₋₁pₓ·qₓ₊ₖ₋₁ (increasing) or Aₓ^(D) = ∑ₖ₌₁^∞ (n+1-k)·v^k·ₖ₋₁pₓ·qₓ₊ₖ₋₁ (decreasing); benefits scale by time or age  
**Purpose:** Align insurance protection with declining liability (mortgage payoff), growing need (family expenses), or cost efficiency  
**Prerequisites:** Present value of benefits, discount factor (v), survival probability (ₚₓ)

## 2. Comparative Framing
| Benefit Pattern | Formula | Typical Use | Example |
|-----------------|---------|------------|---------|
| **Level (Constant)** | B = fixed | Standard protection | $500K term |
| **Increasing (Linear)** | B(t) = B₀ + annual increase | Growing family needs | Start $200K, +$20K/year |
| **Decreasing (Linear)** | B(t) = B₀ - annual decrease | Mortgage payoff | $400K declining to 0 |
| **Geometric Increasing** | B(t) = B₀·(1+i)^t | Inflation protection | $250K growing 3%/year |
| **Stepped Increasing** | B(t) = B₀ if t<n, B₀+ΔB if t≥n | Policy milestone | Double at age 50 |
| **Negatively Convex** | B(t) = max(B₀-t², B_min) | Declining risk | $400K declining rapidly then flat |

## 3. Examples + Counterexamples

**Simple Example:**  
Decreasing term on $400K mortgage, 30-year term: Benefit ≈ $13,300/year; Premium ≈ $45/month (vs $60 for level term)

**Failure Case:**  
Assuming increasing benefit = level benefit × average increase factor: Ignores time-value; overestimates true APV by 20-30%

**Edge Case:**  
Geometrically increasing benefit at 5%/year for 20 years: Grows to $1.3M; premium ≈ 180% of level $500K equivalent (expensive)

## 4. Layer Breakdown
```
Varying Benefit Structure:
├─ Increasing Benefits:
│   ├─ Linear: (IA)ₓ:n̄| = ∑ₖ₌₁ⁿ k·v^k·ₖ₋₁pₓ·qₓ₊ₖ₋₁  (1, 2, 3, ..., n)
│   ├─ Geometric: (IA')ₓ:n̄| = ∑ₖ₌₁ⁿ (1+j)^(k-1)·v^k·ₖ₋₁pₓ·qₓ₊ₖ₋₁  (inflation-adjusted)
│   ├─ Formula relationship: (IA)ₓ:n̄| = [Aₓ:n̄| - n·v^n·ₙpₓ] / i
│   ├─ Continuous: ∫₀ⁿ t·e^{-δt}·ₜpₓ·μₓ₊ₜ dt
│   └─ Applications: Growing family needs, income replacement
├─ Decreasing Benefits:
│   ├─ Linear: (DA)ₓ:n̄| = ∑ₖ₌₁ⁿ (n+1-k)·v^k·ₖ₋₁pₓ·qₓ₊ₖ₋₁  (n, n-1, ..., 1)
│   ├─ Formula relationship: (DA)ₓ:n̄| = [n·Aₓ:n̄| - (IA)ₓ:n̄|] / 1
│   ├─ Proportional: B(t) = B₀·[1 - t/n]  (linear decline to zero)
│   ├─ Constant percentage: B(t) = B₀·(1-r)^t  (exponential decay)
│   └─ Applications: Mortgage payoff, loan protection
├─ Stepped Benefits:
│   ├─ Increase at milestone: B(1-10) = $200K, B(11-20) = $400K, B(21-30) = $200K
│   ├─ Stepped premium: May pay more for higher coverage period
│   ├─ Conversion points: Switch from term to whole life at step boundary
│   └─ Applications: Family coverage (expand with income, contract at retirement)
├─ Multi-Period Structures:
│   ├─ Phase 1 (0-10 years): Level $500K protection
│   ├─ Phase 2 (11-20 years): Decreasing $500K to $250K (mortgage payoff)
│   ├─ Phase 3 (21+ years): Whole life $100K estate coverage
│   └─ Applications: Comprehensive coverage (multiple needs)
├─ Computational Methods:
│   ├─ Discrete sum: Year-by-year calculation
│   ├─ Formula shortcuts: (IA) = [Aₙ̄| - n·v^n·ₙpₓ] / i
│   ├─ Continuous integration: Numerical methods or special tables
│   └─ Commutation functions: Lₓ = v^x·lₓ, Mₓ = ∑ₖ₌ₓ Dₓ simplify calculation
├─ Reserve Implications:
│   ├─ Decreasing: Reserve peaks early, declines to zero at maturity
│   ├─ Increasing: Reserve starts low, grows with benefit
│   ├─ Multiple phases: Reserve steps up/down at phase transitions
│   └─ Surrender value: May follow benefit schedule (decreasing term has nil CSV)
└─ Underwriting & Pricing:
    ├─ Decreasing term: Simplest underwriting (declining risk), lowest cost
    ├─ Increasing term: Higher medical underwriting (adverse selection), higher cost
    ├─ Multi-phase: Careful documentation of phase transitions and benefit levels
    ├─ Anti-selection: Decreasing term less attractive (policyholders drop); increasing more durable
    └─ Profit margins: Decreasing term 10-15%, increasing 20-30%
```

**Key Decision:** Select benefit pattern matching liability structure (mortgage ↔ decreasing, growing family ↔ increasing)

## 5. Mini-Project
Calculate varying benefit premiums, compare to level insurance, and project benefit schedules:
```python
import numpy as np
import pandas as pd
...
## 6. Challenge Round
When varying benefits become problematic:
- **Anti-selection on decreasing**: Healthy customers keep level term; sick cancel decreasing early; remaining cohort dies at higher rate
- **Estimation risk on increasing**: If inflation/raises exceed assumptions, customer perceives inadequate coverage later
- **Basis risk on geometric**: Lock in 3% inflation assumption; if actual 5%, customer underinsured; if actual 1%, customer overpays
- **Lapse incentives on decreasing**: As benefit shrinks, incentive to maintain policy decreases; lapse spike after year 5-10
- **Administrative complexity**: Multiple benefit schedules require tracking, documentation; system errors cause overpayments or underpayments
- **Reformation risk**: Court may void policy if benefit calculation error discovered; insurer appeals or pays claim

## 7. Key References
- [SOA Exam FM Insurance Functions (Chapter 4)](https://www.soa.org/education/exam-req/edu-exam-fm-detail.aspx) - Increasing/decreasing formulas
- [Bowers et al., Actuarial Mathematics (Chapter 5-6)](https://www.soa.org/) - Mathematical derivations
- [Wetterstrand, Mortgage Protection & Decreasing Insurance](https://www.soa.org/) - Practical applications

---
**Status:** Specialized products | **Complements:** Term Insurance, Whole Life, Inflation-Linked Policies
