...
# This notebook-style script provides a runnable mini-project for reserve adequacy.
...
print("Starting topic workflow: reserve adequacy")
...
def calculate_reserve_prospective(
    age, remaining_term, mortality_dict, benefit_amt, annual_premium, interest_rate
):
...
for duration in durations:
    ...
    stat_reserve = calculate_reserve_prospective(
        age_now,
        remaining_term,
        mortality_standard,
        benefit_amount,
    ...
    econ_reserve = calculate_reserve_prospective(
        age_now,
        remaining_term,
        mortality_standard,
        benefit_amount,
    ...
    print(
        f"{duration:<8} {age_now:<8} ${stat_reserve:<14,.0f} ${econ_reserve:<14,.0f} ${held_reserve:<14,.0f} ${shortfall:>+13,.0f} {shortfall_pct:>+10.1f}%"
    )
...
print(
    f"{'Interest Rate':<18} {'Economic Res':<18} {'vs 4% Base':<18} {'Shortfall':<18}"
)
print("-" * 72)

base_econ_res = calculate_reserve_prospective(
    age_test, remaining_test, mortality_standard, benefit_amount, gross_premium, 0.04
)

for rate in interest_rates:
    econ_res = calculate_reserve_prospective(
        age_test,
        remaining_test,
        mortality_standard,
        benefit_amount,
    ...
    pct_vs_base = (
        (((econ_res / base_econ_res) - 1) * 100) if abs(base_econ_res) > 1e-12 else 0.0
    )
    shortfall = econ_res - (stat_reserve)

    print(
        f"{rate*100:>16.2f}% ${econ_res:>16,.0f} {pct_vs_base:>+16.1f}% ${shortfall:>+16,.0f}"
    )
...
econ_res_base = calculate_reserve_prospective(
    age_test,
    remaining_test,
    mortality_standard,
    benefit_amount,
...
econ_res_stressed = calculate_reserve_prospective(
    age_test,
    remaining_test,
    mortality_shocked,
    benefit_amount,
...
print("-" * 66)
stress_pct_vs_base = (
    (((econ_res_stressed / econ_res_base) - 1) * 100)
    if abs(econ_res_base) > 1e-12
    else 0.0
)
print(f"{'Base case (standard mort)':<30} ${econ_res_base:>16,.0f}")
print(
    f"{'Mortality +20%':<30} ${econ_res_stressed:>16,.0f} {stress_pct_vs_base:>+16.1f}%"
)
...
econ_res_compound = calculate_reserve_prospective(
    age_test,
    remaining_test,
    mortality_shocked,
    benefit_amount,
...
print(
    f"Total for cohort: ${(econ_res_compound - econ_res_base) * policies_inforce:,.0f}"
)
...
for rate in interest_rates:
    res = calculate_reserve_prospective(
        age_test,
        remaining_test,
        mortality_standard,
        benefit_amount,
...
ax.axvline(
    x=4.0, color="green", linestyle="--", linewidth=1.5, alpha=0.7, label="Base (4%)"
)
ax.axvline(
    x=2.5,
    color="red",
...
ax.set_title(
    f"Reserve Sensitivity to Interest Rates (Year {year_test})",
    fontsize=12,
    fontweight="bold",
)
ax.legend(fontsize=10)
...
shortfall_pcts = [
    (d["shortfall"] / d["econ_res"] * 100) if d["econ_res"] > 0 else 0
...
bars = ax.bar(
    scenarios_stress, reserves_stress, alpha=0.6, edgecolor="black", linewidth=1.5
)
...
ax.set_ylabel("Reserve per Policy ($)", fontsize=11)
ax.set_title(
    f"Stress Testing Results (Year {year_test}, Age {age_test})",
    fontsize=12,
    fontweight="bold",
)
ax.grid(alpha=0.3, axis="y")
...
# %%
print("Summary complete for topic: reserve adequacy")
