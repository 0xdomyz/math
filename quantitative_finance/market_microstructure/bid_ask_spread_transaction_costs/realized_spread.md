...
## 1. Concept Skeleton
**Definition:** Effective spread minus twice the price change in trade direction, measuring market maker revenue after inventory adjustment  
**Purpose:** Isolates dealer profit from price impact, separates transient from permanent transaction costs  
**Prerequisites:** Effective spread, price impact, market maker inventory management

## 2. Comparative Framing
| Component | Formula | Interpretation | Time Horizon | Typical Sign |
|-----------|---------|----------------|--------------|--------------|
| **Effective Spread** | 2×\|P - Mid₀\| | Total transaction cost | At execution | Positive |
| **Price Impact** | 2×(Mid_t - Mid₀)×Direction | Information revelation | 5-30 minutes | Either sign |
| **Realized Spread** | Effective - Price Impact | Dealer revenue | 5-30 minutes | Usually positive |
| **Adverse Selection** | Price Impact component | Informed trading cost | Post-trade | Positive (informed) |

## 3. Examples + Counterexamples

**Simple Example:**  
Buy at $100.05, Mid₀=$100.00, Mid₅min=$100.02 → Effective=$0.10, Impact=$0.04, Realized=$0.06

**Informed Trade:**  
Buy at $100.05, Mid₀=$100.00, Mid₅min=$100.08 → Effective=$0.10, Impact=$0.16, Realized=-$0.06 (dealer loses)

**Liquidity Trade:**  
Sell at $99.95, Mid₀=$100.00, Mid₅min=$100.03 → Effective=$0.10, Impact=-$0.06, Realized=$0.16 (dealer profits from reversion)

## 4. Layer Breakdown
```
Realized Spread Decomposition:
├─ Measurement Components:
│   ├─ Execution Price (P): Actual trade price
│   ├─ Initial Midpoint (Mid₀): Quote midpoint at trade time
│   ├─ Future Midpoint (Mid_t): Quote midpoint after time t
│   └─ Trade Direction (D): +1 for buy, -1 for sell
├─ Calculation Steps:
│   ├─ Effective Spread: ES = 2 × |P - Mid₀|
│   ├─ Price Change: ΔMid = Mid_t - Mid₀
│   ├─ Price Impact: PI = 2 × ΔMid × D
│   └─ Realized Spread: RS = ES - PI
├─ Economic Interpretation:
│   ├─ RS > 0: Market maker earns revenue (price reverts)
│   ├─ RS = 0: Zero profit (random walk, efficient market)
│   ├─ RS < 0: Market maker loses (adverse selection dominates)
│   └─ RS / ES: Percentage of spread retained by dealer
├─ Time Horizon Selection:
│   ├─ 1-minute: Fast inventory adjustment, high variance
│   ├─ 5-minute: Standard academic/industry benchmark
│   ├─ 30-minute: Captures slower information diffusion
│   └─ 1-day: Long-term permanent vs transient decomposition
└─ Market Maker Strategy:
    ├─ Inventory Management: Adjust quotes to flatten position
    ├─ Adverse Selection Avoidance: Widen spreads during news
    ├─ Order Flow Screening: Identify toxic flow patterns
    └─ Profit Target: Positive realized spread on average
```

**Interaction:** Trade executes → dealer holds inventory → price adjusts → dealer closes position → realized spread determined

## 5. Mini-Project
Decompose effective spreads into realized and impact components:
```python
import numpy as np
...
## 6. Challenge Round
Why might realized spreads be negative, and what does it imply?
- **Adverse selection dominates**: Informed traders consistently move prices against dealers (negative realized = dealer losses)
- **Information events**: During earnings announcements, informed flow increases, dealers lose on inventory
- **Toxic flow detection**: HFT market makers use VPIN to detect adverse selection, widen spreads or withdraw
- **Time horizon matters**: Short horizons may show negative realized (temporary), but longer horizons reveal true profit
- **Market maker risk management**: Negative realized spreads signal need for wider quotes or selective quoting

## 7. Key References
- [Huang & Stoll (1996) - Components of the Bid-Ask Spread](https://www.jstor.org/stable/2329367)
- [Bessembinder & Kaufman (1997) - Realized Spread Comparison](https://onlinelibrary.wiley.com/doi/abs/10.1111/j.1540-6261.1997.tb02744.x)
- [Hendershott & Moulton (2011) - Automation and Realized Spreads](https://www.sciencedirect.com/science/article/abs/pii/S0304405X11000754)
- [Easley et al (2012) - Flow Toxicity and Realized Spreads](https://www.jstor.org/stable/41349501)

---
**Status:** Market maker profitability measure | **Complements:** Effective Spread, Price Impact, Adverse Selection
