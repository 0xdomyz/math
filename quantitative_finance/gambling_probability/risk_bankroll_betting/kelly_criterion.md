...
## 1. Concept Skeleton
**Definition:** Formula determining optimal bet size to maximize long-term bankroll growth while minimizing risk of ruin  
**Purpose:** Balance risk and reward, prevent overbetting, asymptotically maximize expected log wealth  
**Prerequisites:** Expected value, probability, logarithmic growth, optimization

## 2. Comparative Framing
| Strategy | Kelly Criterion | Fixed Fraction | Fixed Unit | All-In |
|----------|-----------------|-----------------|-----------|--------|
| **Bet Size** | f* = (bp - q) / b | Fixed % of bankroll | Fixed amount | Entire bankroll |
| **Growth** | Optimal (exponential) | Suboptimal | Linear | Volatile |
| **Risk** | Minimized | Higher | Higher | Maximized |
| **Drawdown** | Moderate | Larger | Larger | 100% |
| **Use Case** | Professional | Conservative | Casual | Gambling |

## 3. Examples + Counterexamples

**Simple Example:**  
Favorable bet: 55% to win \$110 vs 45% to lose \$100. Kelly suggests betting ~4.5% of bankroll per bet.

**Failure Case:**  
Using Kelly on overestimated edge. If true edge 1% but you estimate 5%, betting 10× too much → rapid ruin.

**Edge Case:**  
Negative EV game (casino): Kelly formula gives negative fraction → don't bet (correct).
...
│   └─ Example: If b=1 (even odds), p=0.55: f* = (0.55 - 0.45) / 1 = 0.10 (10%)
├─ Derivation:
│   ├─ Goal: Maximize E[ln(Wealth)]
│   ├─ With bet size f, final wealth = W(1 + fB) or W(1 - f)
│   ├─ Expected log wealth = p·ln(1+fb) + q·ln(1-f)
│   ├─ Derivative = 0 at optimal f*
│   └─ Result: Kelly formula for fixed odds
├─ Properties:
│   ├─ Asymptotically maximizes long-term growth
│   ├─ Minimizes time to reach any goal
│   ├─ Never goes below full Kelly
│   ├─ Halving bet (f*/2) increases certainty significantly
│   └─ 2× Kelly leads to eventual ruin
├─ Practical Adjustments:
│   ├─ Fractional Kelly (50%, 25%): f = f*/2 or f*/4
│   │   Pro: Reduces variance/drawdown; Con: Slower growth
│   ├─ Kelly with transaction costs: Reduces effective f*
│   ├─ Multiple bets: Use Kelly for portfolio of wagers
│   ├─ Correlated outcomes: Adjustment factor needed
│   └─ Uncertainty in edge: Conservative (fractional) Kelly
├─ Growth Rates:
│   ├─ Kelly: g ≈ p·ln(1+f*b) + q·ln(1-f*)
│   ├─ Growth in bps: ~(edge²/variance) at full Kelly
│   ├─ Bankroll doubling time: ln(2) / g
│   └─ Example: 1% edge → double every 694 bets at Kelly
└─ Violations & Corrections:
    ├─ Violation: Bet > Kelly leads to drawdowns, eventual ruin
    ├─ Violation: Bet < Kelly leaves money on table
    ├─ Correction: Use fractional Kelly if uncertain of edge
    ├─ Correction: Reduce Kelly if outcomes correlated
    └─ Correction: Account for transaction costs in formula
```

## 5. Mini-Project
Calculate Kelly sizes and compare strategies:
```python
import numpy as np
...
print(f"{'Scenario':<30} {'Prob':<10} {'Kelly %':<12} {'1/2 Kelly':<12} {'1/4 Kelly':<12}")
print("-" * 80)

for scenario in scenarios:
    f_kelly = kelly_fraction(scenario["prob"], scenario["odds"])
    print(f"{scenario['name']:<30} {scenario['prob']:<10.1%} {f_kelly*100:<11.2f}% {f_kelly/2*100:<11.2f}% {f_kelly/4*100:<11.2f}%")
...
def simulate_betting(initial_bankroll, prob_win, num_bets, strategy_name, strategy_func):
...
print(f"{'Strategy':<15} {'Mean Final':<15} {'Median':<15} {'Std Dev':<15} {'Ruin Rate':<12}")
print("-" * 70)
...
for name in [s[0] for s in strategies]:
    ...
    print(f"{name:<15} ${mean_val:<14,.0f} ${median_val:<14,.0f} ${std_val:<14,.0f} {ruin_rate:<11.1%}")
...
expected_growth = prob_growth * np.log(1 + f_kelly_sim) + (1 - prob_growth) * np.log(1 - f_kelly_sim)
...
axes[0, 0].plot(edges_plot * 100, np.array(kelly_sizes) * 100, linewidth=2, color='darkgreen')
axes[0, 0].fill_between(edges_plot * 100, np.array(kelly_sizes) * 100, alpha=0.3, color='green')
...
for (label, color) in zip(labels_strat, colors_strat):
    axes[0, 1].hist(results[label]["final_values"], bins=30, alpha=0.5, 
...
for f_kelly_val, label_frac in [(kelly_fraction(prob, 1.0), "Kelly"), 
                                 (kelly_fraction(prob, 1.0)/2, "1/2 Kelly"),
                                 (kelly_fraction(prob, 1.0)/4, "1/4 Kelly")]:
    expected_wealth = initial_bank * np.exp(num_bets_curve * 
                                            (prob * np.log(1 + f_kelly_val) + 
                                             (1-prob) * np.log(1 - f_kelly_val)))
...
axes[1, 1].scatter(ruin_risks[kelly_idx], growth_rates[kelly_idx], 
                   color='red', s=200, marker='*', label='Full Kelly', zorder=5)
...
## 6. Challenge Round
When is full Kelly problematic?
- Estimation error in edge (even 1% overestimate → overbetting)
- Correlated bets (Kelly assumes independence)
- Discrete outcomes (Kelly can require fractional bets)
- Psychological comfort (full Kelly can cause large drawdowns)
- Changing odds environment (Kelly updated per bet)

## 7. Key References
- [Wikipedia: Kelly Criterion](https://en.wikipedia.org/wiki/Kelly_criterion)
- [Kelly Gambling: Original Paper](https://www.jstor.org/stable/2963540)
- [Fortune Formula: Kelly in Practice](https://en.wikipedia.org/wiki/Fortune%27s_Formula)

---
**Status:** Optimal betting framework | **Complements:** Risk of Ruin, Expected Value, Bankroll Management
