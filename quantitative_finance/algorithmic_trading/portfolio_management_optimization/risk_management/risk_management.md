...
## Concept Skeleton
**Definition:** **Definition:** **Definition:** Risk management in algorithmic trading systematically identifies, measures, and controls potential losses through position limits, Value-at-Risk (VaR), stress testing, and stop-loss rules, preventing catastrophic drawdowns while preserving capital for profitable opportunities. Core framework: define risk appetite (maximum acceptable loss), monitor exposures in real-time, and implement automatic safeguards (kill switches, circuit breakers) when thresholds are breached. **Core Components:** - **Position sizing**: Kelly criterion, fixed fractional (risk fixed % of capital per trade), volatility-adjusted sizing - **Exposure limits**: Gross exposure (sum of all positions), net exposure (long - short), sector/factor

**Purpose:**
- Deploy risk management as a repeatable framework for signal-to-execution translation under transaction costs and latency constraints.
