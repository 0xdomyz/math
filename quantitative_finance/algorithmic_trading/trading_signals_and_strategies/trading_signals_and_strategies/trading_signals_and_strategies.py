...
np.random.seed(42)
print("Setup complete for topic: trading signals and strategies")
...
print(f"\nPairs Trading (2Ïƒ entry, 0.5Ïƒ exit):")
...
ax.axhline(2, color='r', linestyle='--', linewidth=1, alpha=0.7, label='Entry (+2Ïƒ)')
ax.axhline(-2, color='g', linestyle='--', linewidth=1, alpha=0.7, label='Entry (-2Ïƒ)')
ax.axhline(0.5, color='orange', linestyle=':', linewidth=1, alpha=0.7)
ax.axhline(-0.5, color='orange', linestyle=':', linewidth=1, alpha=0.7, label='Exit (Â±0.5Ïƒ)')
...
# %%
if 'df' in globals() and 'signal' in df.columns:
    ax = df.plot(x='t', y='signal', figsize=(10, 4), title='trading signals and strategies - Signal View')
