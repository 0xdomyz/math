...
## 1. Concept Skeleton
**Definition:** Annual interest rate i^(m) stated for m compounding periods per year; appears on bonds, mortgages, savings accounts  
**Purpose:** Match market conventions (semi-annual bonds, monthly mortgages), convert to actuarial effective rates, understand rate quotations  
**Prerequisites:** Effective annual rate, compounding frequency, interest calculations, rate conversions

## 2. Comparative Framing
| Rate Basis | Nominal i^(m) | Effective i | Force δ |
|------------|----------------|------------|---------|
| **Stated** | "6% semi-annual" | Usually implicit | Rare in markets |
| **Period rate** | i^(m)/m per period | Applied annually | Infinitesimal |
| **Equivalence** | i^(m) = m[(1+i)^{1/m} - 1] | i = (1 + i^(m)/m)^m - 1 | δ = ln(1+i) |
| **Market use** | Bonds, mortgages, savings | Actuarial, inflation | Stochastic models |
| **Conversion** | i^(m) → i: (1 + i^(m)/m)^m - 1 | i → i^(m): m[(1+i)^{1/m} - 1] | Always between i^(∞) and i |

## 3. Examples + Counterexamples

**Simple Example:**  
6% annual compounded semi-annually = 6%/2 = 3% per half-year = (1.03)² - 1 = 6.09% effective

**Failure Case:**  
Confusing "6% nominal semi-annual" with 6% effective: Actual effective = 6.09%, not 6%; 9 basis points error, 1.5% loss on annuity

**Edge Case:**  
Zero-coupon bond: No stated coupon rate, only maturity value; effective rate derived from price (requires iteration)

## 4. Layer Breakdown
```
Nominal Rate Structure:
├─ Definition:
│   ├─ i^(m) = nominal annual rate, m compounding periods
│   ├─ Period rate = i^(m) / m per period
│   ├─ (1 + i^(m)/m)^m = accumulation over 1 year
│   └─ Effective i = (1 + i^(m)/m)^m - 1
├─ Common Compounding Frequencies:
│   ├─ m=1: Annual (i^(1) = i)
│   ├─ m=2: Semi-annual (typical bonds)
│   ├─ m=4: Quarterly (UK gilts, some bonds)
│   ├─ m=12: Monthly (mortgages, savings accounts)
│   ├─ m=365: Daily (some savings)
│   └─ m→∞: Continuous (theoretical)
├─ Multi-Year Calculations:
│   ├─ n-year accumulation: (1 + i^(m)/m)^{mn}
│   ├─ Semi-annual for 5 years: (1 + i^(2)/2)^{10}
│   ├─ Monthly for 30 years: (1 + i^(12)/12)^{360}
│   └─ Fractional period: interpolate (1 + i^(m)/m)^t
├─ Bond Pricing (uses nominal rates):
│   ├─ Coupon rate: quoted as nominal semi-annual
│   ├─ Spot curve: expressed as nominal annual semi-annual
│   ├─ Price = ∑ Coupon·v^t + Face·v^n
│   └─ Yield-to-maturity: solve for i to equate price
└─ Actuarial Conversion:
    ├─ Insurance premium paid annually but i^(12) market rate
    ├─ Mortgage 5% annual compounded monthly = i^(12) = 5%
    ├─ Convert to effective: i = (1 + 0.05/12)^{12} - 1 ≈ 5.116%
    └─ Use effective for all PV calculations
```

**Interaction:** Market quotes i^(m) → Convert to effective i → Use in actuarial formulas → Output in whatever format required

## 5. Mini-Project
Convert between nominal and effective rates, price bonds, and calculate mortgages:
```python
import numpy as np
import pandas as pd
...
## 6. Challenge Round
When nominal rates mislead:
- **Basis point confusion**: "6% semi-annual" ≠ "6% annual"; ambiguity in market conventions
- **Frequency mismatch**: Treasury yields quoted annual semi-annual, swap spreads quarterly; must convert to common base
- **Floating rate resets**: Mortgage tied to LIBOR + spread, compounded monthly; actual rate varies with index changes
- **Negative nominal rates**: ECB deposits at -0.75% effective; breaks intuition (paying to save), requires careful modeling
- **Term premium embedded**: Spot curve isn't flat; long-term nominal rates include risk premium; ignoring creates model error

## 7. Key References
- [Bond Mathematics (Wikipedia)](https://en.wikipedia.org/wiki/Bond_(finance)#Valuation) - Pricing conventions
- [Bowers et al., Actuarial Mathematics (Chapter 2)](https://www.soa.org/) - Rate conversion formulas
- [UK Gilt-Edged Market](https://www.dmo.gov.uk/) - Quarterly compounding conventions

---
**Status:** Market-standard rate quotation | **Complements:** Effective Annual Rate, Force of Interest, Bond Pricing
