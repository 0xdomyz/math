...
class RiskNeutralPricing:
    ...
    def price_european_analytical(self, K, T, option_type="call"):
        ...
        d1 = (np.log(self.S0 / K) + (self.r + 0.5 * self.sigma**2) * T) / (
    ...
    def radon_nikodym_derivative(self, S_T, T, mu):
        ...
        W_T = (np.log(S_T / self.S0) - (mu - 0.5 * self.sigma**2) * T) / (
            self.sigma * np.sqrt(T)
        )
...
print(
    f"  ✓ Martingale property verified"
...
# Under P
prob_below_P = norm.cdf(
    (np.log(K_low / S0) - (mu_real - 0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
)
prob_above_P = 1 - norm.cdf(
    (np.log(K_high / S0) - (mu_real - 0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
)

# Under Q
prob_below_Q = norm.cdf(
    (np.log(K_low / S0) - (r - 0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
)
prob_above_Q = 1 - norm.cdf(
    (np.log(K_high / S0) - (r - 0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
)
