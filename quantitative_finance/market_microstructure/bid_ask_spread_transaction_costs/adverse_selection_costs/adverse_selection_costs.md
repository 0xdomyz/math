...
## 1. Concept Skeleton
**Definition:** Transaction cost arising from trading with better-informed counterparties who exploit information asymmetry  
**Purpose:** Quantifies cost of information disadvantage, explains dealer spread widening, drives market maker risk management  
**Prerequisites:** Information asymmetry, market microstructure, dealer models (Glosten-Milgrom, Kyle)

## 2. Comparative Framing
| Cost Component | Adverse Selection | Inventory Cost | Order Processing | Liquidity Premium |
|----------------|-------------------|----------------|------------------|-------------------|
| **Nature** | Informational disadvantage | Risk-bearing | Fixed overhead | Immediacy service |
| **Permanence** | Permanent price impact | Transient (mean-reverts) | Per-transaction | Varies with depth |
| **Driver** | Informed traders | Position volatility | Infrastructure | Impatience |
| **% of Spread** | 40-70% (typical) | 10-30% | 10-20% | Varies |

## 3. Examples + Counterexamples

**Simple Example:**  
Insider knows merger announcement, buys stock. Dealer sells at $50, price jumps to $60 post-announcement → $10 adverse selection loss

**No Adverse Selection:**  
Index fund rebalancing (known mechanical flow), no private information → dealer profits from bid-ask spread, price reverts

**Edge Case:**  
Earnings announcement leak: Informed traders flood market → dealers widen spreads 3x, refuse to trade large sizes

## 4. Layer Breakdown
```
Adverse Selection Mechanism:
├─ Information Structure:
│   ├─ Private Information: Insiders, analysts, proprietary models
│   ├─ Public Information: News, earnings, macro data
│   ├─ Information Quality: Signal precision, timeliness
│   └─ Information Asymmetry: Gap between informed and uninformed
├─ Dealer Problem:
│   ├─ Inference: Distinguish informed from uninformed flow
│   ├─ Protection: Widen spread when adverse selection high
│   ├─ Inventory Risk: Exposed to directional informed trades
│   └─ Learning: Update beliefs from order flow patterns
├─ Glosten-Milgrom Model:
│   ├─ Assumptions: Sequential trade, binary information
│   ├─ Spread Setting: Bid/ask bracket true value expectation
│   ├─ Bayesian Updating: Posterior adjusted after each trade
│   └─ Equilibrium: Spread widens with informed trader probability
├─ Measurement Approaches:
│   ├─ Price Impact: Permanent component of execution cost
│   ├─ PIN (Easley et al): Probability of informed trading
│   ├─ VPIN: Volume-synchronized PIN for HFT environment
│   └─ Spread Decomposition: Huang-Stoll, Glosten-Harris methods
└─ Market Maker Response:
    ├─ Spread Widening: Increase during high information events
    ├─ Size Restrictions: Limit large orders (potential informed)
    ├─ Fast Cancelation: Pull quotes when toxicity detected
    └─ Dynamic Pricing: Real-time adjustment to flow toxicity
```

**Interaction:** Informed trader → trade → dealer loses → spread widens → informed profit declines → equilibrium

## 5. Mini-Project
Simulate adverse selection and measure its components:
```python
import numpy as np
...
## 6. Challenge Round
How do market makers protect against adverse selection?
- **Dynamic spread adjustment**: Widen quotes during earnings, news events, or detected toxic flow (VPIN monitoring)
- **Size-based pricing**: Larger orders pay wider spreads (potential informed traders avoid revealing information through size)
- **Fast quote updates**: HFT market makers cancel stale quotes in microseconds (latency arbitrage protection)
- **Order flow segmentation**: Differentiate retail (uninformed) vs institutional (potentially informed) flow
- **Machine learning**: Classify orders as toxic/non-toxic using historical patterns, microstructure signals

## 7. Key References
- [Glosten & Milgrom (1985) - Bid-Ask Spread Sequential Trade Model](https://www.sciencedirect.com/science/article/abs/pii/0304405X85900443)
- [Kyle (1985) - Continuous Auctions and Insider Trading](https://www.jstor.org/stable/1913210)
- [Easley et al (1996) - Liquidity, Information, and PIN](https://www.jstor.org/stable/2329394)
- [Easley et al (2012) - Flow Toxicity and VPIN](https://www.jstor.org/stable/41349501)

---
**Status:** Core information asymmetry cost | **Complements:** Bid-Ask Spread, Price Impact, Market Maker Models
