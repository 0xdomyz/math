...
if RV_naive > IV_true:
    print(f"  âœ— Upward bias due to microstructure noise")
...
if abs(RK_bartlett - IV_true) < abs(RV_naive - IV_true):
    ...
    print(f"  âœ“ {improvement:.1f}% improvement over naive RV")
...
if p_value < 0.05:
    print(f"  âœ“ Reject Hâ‚€: Jumps detected (5% level)")
else:
    print(f"  âœ— Cannot reject Hâ‚€: No significant jumps")
...
axes[2, 1].set_ylabel('Autocovariance Î³(h)')
axes[2, 1].set_title('Return Autocovariance (Noise â†’ Negative Î³(1))')
...
# HAR model: RV_t = Î²0 + Î²_DÂ·RV_{t-1} + Î²_WÂ·RV_{t-5:t-1} + Î²_MÂ·RV_{t-22:t-1}
...
print(f"    Î²_D (daily): {har_model.coef_[0]:.4f}")
print(f"    Î²_W (weekly): {har_model.coef_[1]:.4f}")
print(f"    Î²_M (monthly): {har_model.coef_[2]:.4f}")
print(f"  Intercept: {har_model.intercept_:.6f}")
print(f"\n  Out-of-sample RÂ²: {r2_har:.4f}")
print(f"  MSE: {mse_har:.6f}")

if r2_har > 0.3:
    print(f"  âœ“ HAR captures volatility persistence well")
...
print(f"   â†’ Noise-robust estimator critical at high frequency")

print("\n2. Optimal Sampling:")
print(f"   Signature plot shows minimum around 5-15 minutes")
print(f"   â†’ Trade-off between precision and noise")

print("\n3. Jump Detection:")
print(f"   True jumps: {n_jumps}")
print(f"   Jump test p-value: {p_value:.4f}")
if p_value < 0.05 and n_jumps > 0:
    print(f"   âœ“ Test correctly detects jumps")
elif p_value >= 0.05 and n_jumps == 0:
    print(f"   âœ“ Test correctly finds no jumps")

print("\n4. Forecasting:")
print(f"   HAR model RÂ²: {r2_har:.3f}")
print(f"   â†’ Multi-horizon aggregation captures persistence")

print("\n5. Practical Recommendations:")
print("   â€¢ Use 5-minute sampling for liquid assets")
print("   â€¢ Apply realized kernel for very high frequency")
print("   â€¢ Test for jumps; use bipower if significant")
print("   â€¢ HAR model simple and effective for forecasting")
print("   â€¢ Store realized measures for GARCH evaluation")

print("\n6. Applications:")
print("   â€¢ Risk management: Superior volatility proxy")
print("   â€¢ Trading: Variance swap valuation")
print("   â€¢ Microstructure: Noise-to-signal ratio")
print("   â€¢ Academic: Model-free benchmark")
