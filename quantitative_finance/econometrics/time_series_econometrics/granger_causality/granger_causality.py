...
# Generate synthetic money supply and inflation (money â†’ inflation with lag)
np.random.seed(42)
periods = 200
...
print("="*70)
print("GRANGER CAUSALITY: Money Supply â†’ Inflation")
print("="*70)
...
# 2. Granger causality test: Money â†’ Inflation
print("\n" + "="*70)
print("2. GRANGER CAUSALITY TEST (Differenced Data)")
print("="*70)

print("\nHâ‚€: Money Supply does NOT Granger-cause Inflation")
print("-"*70)
...
print("\nHâ‚€: Inflation does NOT Granger-cause Money Supply")
print("-"*70)
...
print("\n" + "="*70)
print("5. IMPULSE RESPONSE: Money Shock â†’ Inflation")
print("="*70)
...
ax = axes[0, 1]
ax.plot(data_diff.index, data_diff['money_supply'], 'b-', linewidth=1, label='Î”Money', alpha=0.7)
ax.plot(data_diff.index, data_diff['inflation'], 'r-', linewidth=1, label='Î”Inflation', alpha=0.7)
...
ax.plot(x_line, y_line, 'r--', linewidth=2, label=f'Î²={slope:.4f}, RÂ²={r_value**2:.3f}')
ax.set_xlabel('Î”Money (t-1)')
ax.set_ylabel('Î”Inflation (t)')
...
ax = axes[1, 0]
horizons = range(10)
irf_money_on_infl = irf.irfs[:, 1, 0]  # Money shock â†’ Inflation
...
ax.set_title('Impulse Response: Money Shock â†’ Inflation')
ax.grid(alpha=0.3)
...
1. Money Granger-causes Inflation:
   â†’ Past money supply predicts inflation beyond past inflation alone
   â†’ Supports monetary theory of inflation
   
2. VAR coefficients show:
   â†’ Inflation responds to lagged money with {infl_eq_coefs.iloc[2]:.4f} elasticity
   â†’ Money supply responds to lagged inflation with {money_eq_coefs.iloc[3]:.4f} elasticity
   
3. Impulse response shows:
   â†’ Money shock impact peaks at quarter {np.argmax(irf_money_on_infl)}
   â†’ Cumulative effect: {irf_money_on_infl.sum():.4f}
   
4. FEVD shows:
   â†’ Money explains {money_contrib[-1]*100:.1f}% of inflation variance
   â†’ But causation vs correlation unclear (both respond to expectations)
   
5. Important caveats:
   â†’ Granger causality â‰  True causality
   â†’ Both could respond to central bank credibility announcements
   â†’ Use as diagnostic tool, not causal proof
""")
