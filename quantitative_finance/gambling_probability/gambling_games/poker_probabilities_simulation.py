"""
Poker Probabilities Analysis: execution and visualizations.
Extracted from: poker_probabilities.py
"""

import matplotlib.pyplot as plt
import numpy as np
from poker_probabilities_models import calculate_hand_equity_simulation, evaluate_hand
...
def run():
    ...
    print(
        f"{'Draw Type':<30} {'Outs':<10} {'Turn %':<12} {'River %':<12} {'Either %':<12}"
    )
    print("-" * 76)
    ...
    axes[0, 1].bar(
        x_pos + 0.2, river_probs, 0.4, label="River", alpha=0.7, color="orange"
    )
    ...
    for outs, name, color in draw_options:
        ...
        axes[1, 0].axhline(
            ...
            linestyle="--",
            alpha=0.5,
            label=f"{name}: {equity:.0f}%",
        )

    axes[1, 0].fill_between(
        pot_odds_range, breakeven_equity, 50, alpha=0.2, color="green"
    )
