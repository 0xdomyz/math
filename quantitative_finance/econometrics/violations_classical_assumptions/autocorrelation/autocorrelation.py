...
# True model: Y = 10 + 2*X + Îµ (with AR(1) errors)
Y = 10 + 2*X + epsilon
...
print(f"  True Model: Y = 10 + 2*X + Îµ")
print(f"  Error Structure: Îµâ‚œ = {rho}*Îµâ‚œâ‚‹â‚ + uâ‚œ (AR(1))")
...
axes[0, 1].set_xlabel('Îµâ‚œâ‚‹â‚')
axes[0, 1].set_ylabel('Îµâ‚œ')
axes[0, 1].set_title(f'Îµâ‚œ vs Îµâ‚œâ‚‹â‚ (Positive Correlation)')
...
axes[2, 0].plot(df['t'], df['epsilon_true'], label='True Îµ', alpha=0.7)
...
if dw_stat < 1.5:
    print("  âœ“ Evidence of positive autocorrelation")
elif dw_stat > 2.5:
...
rho_dw = 1 - dw_stat / 2
print(f"  Implied ÏÌ‚ â‰ˆ 1 - DW/2 = {rho_dw:.4f} (True Ï: {rho})")

# Breusch-Godfrey Test
bg_test = acorr_breusch_godfrey(ols_model, nlags=4)
print(f"\nBreusch-Godfrey LM Test (4 lags):")
print(f"  Hâ‚€: No autocorrelation up to lag 4")
...
if bg_test[1] < 0.05:
    print("  âœ“ Reject Hâ‚€: Autocorrelation detected")
else:
...
if significant_lags:
    print(f"  âœ“ Significant autocorrelation at lags: {significant_lags}")
else:
...
# Step 2: Estimate Ï from residuals
...
print(f"Estimated Ï from OLS residuals: {rho_hat:.4f} (True: {rho})")
...
print(f"  Estimated Ï: {glsar_model.rho:.4f}")
print(f"\nCoefficients:")
...
print(f"  {'âœ“ Improved (closer to 2)' if abs(dw_co - 2) < abs(dw_stat - 2) else 'âš  Not improved'}")
...
print(f"  OLS (incorrect SE):  {coverage_ols:.3f}  {'âœ—' if coverage_ols < 0.90 else 'âœ“'}")
print(f"  OLS + HAC:           {coverage_hac:.3f}  {'âœ“' if 0.94 <= coverage_hac <= 0.96 else 'âš '}")
print(f"  Cochrane-Orcutt:     {coverage_co:.3f}  {'âœ“' if 0.94 <= coverage_co <= 0.96 else 'âš '}")
...
print("\n1. Detection:")
print(f"   â€¢ Visual: Residual time plot shows persistence")
print(f"   â€¢ DW: {dw_stat:.2f} < 2 indicates positive autocorrelation")
print(f"   â€¢ BG test: p < 0.001, strong evidence")

print("\n2. Consequences of Ignoring:")
print(f"   â€¢ Coefficients: Unbiased but inefficient")
print(f"   â€¢ Standard errors: Underestimated by ~{(ols_hac.bse['X'] - ols_model.bse['X'])/ols_model.bse['X']*100:.1f}%")
print(f"   â€¢ Coverage rate: {coverage_ols:.1%} instead of 95%")

print("\n3. Correction Methods:")
print("   â€¢ HAC (Newey-West): Always safe, valid inference")
print("   â€¢ Cochrane-Orcutt: Efficiency gain if AR(1) structure correct")
print("   â€¢ Prais-Winsten: Better than C-O (retains first observation)")

print("\n4. Practical Recommendations:")
print("   â€¢ Time series data: Always check for autocorrelation")
print("   â€¢ Default: Use Newey-West HAC standard errors")
print("   â€¢ Strong AR pattern: Consider dynamic specification (lagged Y)")
print("   â€¢ Panel data: Use clustered SE by entity")
print("   â€¢ Model diagnostics: Check ACF of final residuals")
